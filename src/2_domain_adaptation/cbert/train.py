...
from models import BertForCL, RobertaForCL  # type: ignore
...
from transformers import default_data_collator  # isort:skip
from transformers import CONFIG_MAPPING, MODEL_FOR_MASKED_LM_MAPPING, AutoConfig, set_seed  # isort:skip

from transformers import TrainingArguments  # isort:skip

from transformers import HfArgumentParser  # isort:skip

from transformers import AutoTokenizer  # isort:skip

from transformers import BertForPreTraining  # isort:skip
...
class ModelArguments:
    ...
    do_mtr: bool = field(default=False, metadata={"help": "Whether to use MTR auxiliary objective."})
    ...
    mtr_weight: float = field(
        default=0.33,
        metadata={"help": "Weight for MTR auxiliary objective (only effective if --do_mtr)."},
    )
    ...
    num_mtr_labels: Optional[int] = field(default=210, metadata={"help": "Num of PhyChem properties as MTR labels."})
...
class DataTrainingArguments:
    ...
    normalization_path: Optional[str] = field(
        default="data/guacamol_normalization_values_207.json",
        metadata={"help": "Path to mean and std of a dataset for normalization"},
    )
...
def main():
    ...
    def _compute_descriptors(smiles):
        ...
        mol_descriptors = physicochemical_fingerprint_extractor.compute_descriptors(smiles)
