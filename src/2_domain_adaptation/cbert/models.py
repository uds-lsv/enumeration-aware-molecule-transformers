...
class Pooler(nn.Module):
    ...
    def forward(self, attention_mask, outputs):
        ...
        if self.pooler_type in ["cls_before_pooler", "cls"]:
            ...
            return ((first_hidden + last_hidden) / 2.0 * attention_mask.unsqueeze(-1)).sum(1) / attention_mask.sum(
                -1
            ).unsqueeze(-1)
...
def cl_forward(
    ...
    if mtr_input_ids is not None:
        ...
        outputs = encoder(
            ...
            output_hidden_states=cls.model_args.pooler_type in ["avg_top2", "avg_first_last"],
            return_dict=True,
        )
    ...
    if mlm_outputs is not None and mlm_labels is not None:
        ...
        loss = (cls.model_args.cl_weight * loss) + (cls.model_args.mlm_weight * masked_lm_loss)
...
def sentemb_forward(
    ...
    outputs = encoder(
        ...
        inputs_embeds=inputs_embeds,
        output_attentions=output_attentions,
        output_hidden_states=cls.pooler_type in ["avg_top2", "avg_first_last"],
        return_dict=True,
    )
...
class BertForCL(BertPreTrainedModel):
    ...
    def __init__(self, config, *model_args, **model_kargs):
        ...
        if self.model_args.do_mtr:
            self.mtr_head = BertRegressionHead(config)
